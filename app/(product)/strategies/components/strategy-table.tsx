...
export const StrategyTable = observer(({
  ...
  const [editingStrategy, setEditingStrategy] = useState<StrategyWithCounts | null>(null);
  ...
  const onSubmit = async (data: EditStrategyFormData) => {
    ...
    try {
      ...
      console.log("Strategy update requested:", { id: editingStrategy.id, data });
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <Badge
            variant="outline"
            className={`${config.color} ${config.hover} transition-all duration-200 transform hover:scale-105 select-none min-w-[90px] justify-center gap-1 py-1.5 cursor-pointer ${
              actionLoading === strategyId ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-md'
            }`}
    ...
    return (
      <Card className="bg-red-50 border-red-200">
        <CardContent className="p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-red-800 mb-2">Error Loading Strategies</h3>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex flex-col sm:flex-row gap-4 flex-1">
            ...
            {/* Asset Class Filter */}
            <Select value={assetClassFilter} onValueChange={onAssetClassFilterChange}>
              <SelectTrigger className="w-48">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <Select value={strategyTypeFilter} onValueChange={onStrategyTypeFilterChange}>
              <SelectTrigger className="w-48">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <TableBody>
              {isLoading ? (
                ...
                strategies.map((strategy) => (
                  <TableRow 
                    key={strategy.id} 
                    className={`hover:bg-muted/50 ${highlightNewRows ? 'animate-pulse' : ''}`}
                  >
                    ...
                    <TableCell>
                      <Badge 
                        ...
                        className={`text-xs font-medium ${
                          strategy.assetClass === 'EQUITY' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                          strategy.assetClass === 'DERIVATIVES' ? 'bg-purple-50 text-purple-700 border-purple-200' :
                          strategy.assetClass === 'CRYPTO' ? 'bg-orange-50 text-orange-700 border-orange-200' :
                          strategy.assetClass === 'COMMODITIES' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                          strategy.assetClass === 'FOREX' ? 'bg-green-50 text-green-700 border-green-200' :
