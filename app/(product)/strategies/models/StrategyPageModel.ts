...
export class StrategyPageModel {
  ...
  get filteredStrategies(): StrategyWithCounts[] {
    ...
    if (this.filters.statusFilter !== "all") {
      filtered = filtered.filter(strategy => 
        strategy.status === this.filters.statusFilter
      );
    }
    
    if (this.filters.assetClassFilter !== "all") {
      filtered = filtered.filter(strategy => 
        strategy.assetClass === this.filters.assetClassFilter
      );
    }
    
    if (this.filters.strategyTypeFilter !== "all") {
      filtered = filtered.filter(strategy => 
        strategy.strategyType === this.filters.strategyTypeFilter
      );
    }
  ...
  async fetchStrategies() {
    ...
    try {
      ...
      runInAction(() => {
        ...
        this.pagination.totalPages = Math.ceil(userStrategies.length / this.pagination.pageSize);
        this.isLoadingStrategies = false;
      });
      
    } catch (error) {
      runInAction(() => {
        this.strategiesError = error instanceof Error ? error.message : 'Failed to fetch strategies';
        this.isLoadingStrategies = false;
      });
    }
  }
  ...
  async fetchSummary() {
    ...
    try {
      ...
      const pausedStrategies = this.strategies.filter(s => s.status === 'PAUSED').length;
      const stoppedStrategies = this.strategies.filter(s => s.status === 'STOPPED').length;
      
      const totalPnl = this.strategies.reduce((sum, s) => sum + (s.totalPnl || 0), 0);
      const totalTrades = this.strategies.reduce((sum, s) => sum + (s.totalTrades || 0), 0);
      const winRate = totalStrategies > 0 
        ? this.strategies.reduce((sum, s) => sum + (s.winRate || 0), 0) / totalStrategies
        : 0;
  ...
  async activateStrategy(strategyId: string, allocationAmount: number = 0) {
    try {
      const result = await tradingEngineApi.activateStrategy(strategyId, allocationAmount);
