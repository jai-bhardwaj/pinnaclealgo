...
type CreateUserData = Omit<User, 'id' | 'createdAt' | 'updatedAt' | 'hashedPassword'> & {
    password: string;
};

type UpdateUserData = Partial<Omit<User, 'id' | 'createdAt' | 'updatedAt' | 'hashedPassword'>>;

// User CRUD operations
export const createUser = async (userData: CreateUserData): Promise<UserWithRelations> => {
...
export const getUserByUsername = async (username: string): Promise<UserWithRelations | null> => {
    return prisma.user.findUnique({
        where: { username },
        include: {
            profile: true,
...
export const updateUser = async (id: string, userData: UpdateUserData): Promise<UserWithRelations> => {
...
export const deactivateUser = async (id: string): Promise<UserWithRelations> => {
    return prisma.user.update({
        where: { id },
...
export const suspendUser = async (id: string, reason?: string): Promise<UserWithRelations> => {
    return prisma.user.update({
        where: { id },
        data: {
...
export const changeUserRole = async (id: string, role: UserRole): Promise<UserWithRelations> => {
    return prisma.user.update({
        where: { id },
        data: { role },
        include: {
...
export const enableTwoFactor = async (id: string): Promise<{ secret: string; qrCode: string }> => {
    // Implementation would use a 2FA library like speakeasy
    const secret = 'generated-secret'; // Generate actual secret
    const qrCode = 'qr-code-data'; // Generate QR code
...
export const verifyTwoFactor = async (id: string, token: string): Promise<UserWithRelations> => {
    const user = await prisma.user.findUnique({
        where: { id },
    ...
    // Verify token with speakeasy or similar library
...
export const disableTwoFactor = async (id: string, token: string): Promise<UserWithRelations> => {
    // Verify token first, then disable
    return prisma.user.update({
        where: { id },
        data: {
...
export const verifyEmail = async (token: string): Promise<UserWithRelations> => {
...
export const getUserStats = async (id: string) => {
    const [user, orderCount, tradeCount, strategyCount, balance] = await Promise.all([
...
// Search users
export const searchUsers = async (query: string, filters?: FilterParams): Promise<UserWithRelations[]> => {
    const where: any = {
        OR: [
...
export const bulkDeleteUsers = async (userIds: string[]): Promise<{ count: number }> => {
