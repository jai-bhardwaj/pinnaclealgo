...
const getUserId = (ctx: any): string => {
  // This should be replaced with actual session logic when NextAuth is properly configured
  // For now, we throw an error to ensure proper authentication is implemented
  throw new TRPCError({
    code: "UNAUTHORIZED",
    message:
      "Authentication required. Please implement proper session handling.",
  });
};
...
export const strategyRouter = createTRPCRouter({
      ...
      try {
        ...
        throw new TRPCError({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to square off strategy",
        });
      }
    }),

  // Square off all strategies
  squareOffAll: protectedProcedure.mutation(async ({ ctx }) => {
    try {
      const userId = getUserId(ctx);

      // Get all active strategies for the user
      const result = await strategyService.getStrategiesByUserId(
        userId,
        undefined,
        {
      ...
      throw new TRPCError({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to square off all strategies",
      });
    }
  }),
  ...
  bulkUpdate: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        return await strategyService.bulkUpdateStrategies(
          input.strategyIds,
          input.updateData
        );
      } catch (error) {
        throw new TRPCError({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to bulk update strategies",
        });
      }
    }),
      ...
      try {
        ...
        throw new TRPCError({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to bulk delete strategies",
        });
      }
    }),
  ...
  clone: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        return await strategyService.cloneStrategy(
          input.id,
          input.newName,
          input.userId
        );
