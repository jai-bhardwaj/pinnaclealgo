...
import type {
    ...
    StrategyWithCounts,
    StrategyPerformance,
    StrategyStats, StrategyFilters, StoreState
} from '@/types';
...
export class StrategyStore implements StoreState {
    ...
    async updateStrategy(id: string, data: Partial<Strategy>) {
        ...
        try {
            ...
            const updatedStrategy = await vanillaTrpc.strategy.update.mutate({ id, ...filteredData });
            runInAction(() => {
                console.log('Strategy updated successfully:', updatedStrategy);
                // Since the update mutation returns a strategy that matches StrategyWithCounts structure,
                // we can safely add the required _count property if it's missing
                const strategyWithCount: StrategyWithCounts = {
                    ...updatedStrategy,
                    _count: updatedStrategy._count || { orders: 0, backtests: 0 }
                };
                this.updateStrategyInList(strategyWithCount);
            });
    ...
    updateStrategyInList(updatedStrategy: StrategyWithCounts) {
        const index = this.strategies.findIndex((strategy) => strategy.id === updatedStrategy.id);
        if (index !== -1) {
    ...
    removeStrategies(strategyIds: string[]) {
        this.strategies = this.strategies.filter((strategy) => !strategyIds.includes(strategy.id));
        this.strategiesTotal = Math.max(0, this.strategiesTotal - strategyIds.length);
